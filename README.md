...
135. 把整数转化成excel的col(LeetCode171, easy)
关键在append之前要--；最好的方法在源代码里

136. 一个有重复元素的数组的子集(LeetCode 90, medium)
基本是循环回溯的应用，对于重复元素计数。好方法+解释放在源代码里

137. 反转一个链表从位置m到n的所有节点(LeetCode92, medium)
找另外一个链表头，一个一个插进去就好。反转的时候就只在链表头插

138. 把链表k个组成一组，每一组内部反转(LeetCode25, hard)
基本套用了92的思想。用了两个临时表。原始表的节点倒着插进临时表1，然后判断是否够k个节点，够就直接连到临时表2的末尾。否则就要再次倒着插回末尾。

139. 深度拷贝一个带随机指针的链表(LeetCode138, hard)
用hashset的方法太简单不说了。只说O(1) Space的方式
...
141. 课程设计，实际上是求一个有向图的无环遍历(LeetCode210, medium)
基本和207一样，把轨迹保存一下就好

142. 从前序和中序遍历重建二叉树(LeetCode105, medium)
一个递归，基本就是模拟下人的做法：在前序里找到根，在中序里二分找到左子树和右子树，重复这个过程即可

143. 按层遍历二叉树(LeetCode 102, easy)
没什么好说的，队列遍历即可

144. 雨水收集(LeetCode hard)
我的方法：O(NlgN)二分。在左和右之间找到一个比左或右大的元素，然后在这个元素两边二分即可。找不到这样的元素就直接return count了。
更好的方法：和之前一个很像。从小的一边往中间走，下一个比自己大就更新，否则就计算trap到的雨水量。O(N)

145. O(NlgN)时间排序一个链表(LeetCode148, medium)
归并是比较容易的，关键是partition部分也变成了O(NlgN)。主要还是龟兔赛跑让一个指针指到中点，然后分成两条链合并即可。

146. 找到1-n的第k个排列(LeetCode 60, medium)
一个完全不同的思路：从最高位到最低位一位一位的确定。首先确定这一位是第几个数，然后在数组里把这个数设为0。减掉这个order继续下一位

147. 自底向上按层遍历一棵树(LeetCode107, easy)
这题和102有什么区别。。。我只加了两个字符就ac了。。。

148. 求一个英语句子的最后一个单词的长度(LeetCode58, easy)
先trim，然后找前一个空格，用长度减位置再减1即可。

149. 把一个排好序的链表转成平衡二叉搜索树(LeetCode109, medium)
基本和归并排序一样，先找到中间元素，然后就变成数组了。更好的办法是先左子树再根再右子树，左子树n/2，右子树n - n/2 - 1

150. 段落两端对齐(LeetCode68, hard)
让人极度无语的testcase。。。主要是空格的处理麻烦点。好一点的代码放进去了
...
153. 快乐数字 (LeetCode202, easy)
龟兔赛跑，出现了好几遍，值得背

154. 归并数组 (LeetCode88, easy)
应该倒着来。。。

155. 把数字每一位加起来返回一个一位数(LeetCode258, easy)
找规律：0->0, 9->9, 其他是n % 9
